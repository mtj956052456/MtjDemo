package com.example.pmprogect.fragment.home.multithreading.download.service;
...
import com.example.pmprogect.fragment.home.multithreading.download.bean.DownloadInfo;
import com.example.pmprogect.fragment.home.multithreading.download.bean.FileInfo;
import com.example.pmprogect.fragment.home.multithreading.download.bean.RequestInfo;
import com.example.pmprogect.fragment.home.multithreading.download.config.InnerConstant;
import com.example.pmprogect.fragment.home.multithreading.download.db.DbHolder;
import com.example.pmprogect.fragment.home.multithreading.download.execute.DownloadExecutor;
import com.example.pmprogect.fragment.home.multithreading.download.execute.DownloadTask;
import com.example.pmprogect.fragment.home.multithreading.download.utils.LogUtils;
...
public class DownloadService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (canRequest) {
            ...
            if (null != intent && intent.hasExtra(InnerConstant.Inner.SERVICE_INTENT_EXTRA)) {
                try {
                    ArrayList<RequestInfo> requesetes = (ArrayList<RequestInfo>) intent.getSerializableExtra(InnerConstant.Inner.SERVICE_INTENT_EXTRA);
    ...
    private synchronized void executeDownload(RequestInfo requestInfo) {
        ...
        //先看看在任务列表里，是否有这个任务
        String uniqueId = mDownloadInfo.getUniqueId();
        DownloadTask task = mTasks.get(uniqueId);
        ...
        if (null == task) { //之前没有类似任务
            if (null != mFileInfo) {
                if (mFileInfo.getDownloadStatus() == DownloadStatus.LOADING || mFileInfo.getDownloadStatus() == DownloadStatus.PREPARE) {
                    //修正文件状态
                    ...
                    if (mDownloadInfo.getFile().exists()) {
                        if (!TextUtils.isEmpty(mDownloadInfo.getAction())) {
                            ...
                            intent.putExtra(InnerConstant.EXTRA_INTENT_DOWNLOAD, mFileInfo);
...
//            if (task.getStatus() == DownloadStatus.LOADING || task.getStatus() == DownloadStatus.PAUSE) {
//                mExecutor.executeTask(task);
//            } else if (task.getStatus() == DownloadStatus.COMPLETE) {
//                if (!TextUtils.isEmpty(mDownloadInfo.getAction())) {
//                    Intent intent = new Intent();
//                    intent.setAction(mDownloadInfo.getAction());
//                    intent.putExtra(InnerConstant.EXTRA_INTENT_DOWNLOAD, mFileInfo);
